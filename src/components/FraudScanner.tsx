      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full p-6 shadow-2xl relative max-h-[85vh] overflow-hidden flex flex-col animate-scale-in">
        ...
        <div className="flex items-start justify-between pb-4 border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-2.5">
            <span className="p-2 bg-indigo-500/10 rounded-xl relative">
              <History className="h-5 w-5 text-indigo-400" />
            </span>
            <div>
              <h3 className="text-base font-extrabold text-white">📋 Riwayat Auditor</h3>
              <p className="text-xs text-slate-400 mt-0.5">Daftar seluruh notifikasi yang pernah ditutup / diselesaikan.</p>
            </div>
          </div>
          <button 
            type="button" 
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1.5">
          {dismissedAlerts.length === 0 ? (
            <div className="text-center py-12 text-slate-500 flex flex-col items-center justify-center gap-2">
              ...
              <p className="font-extrabold text-xs text-slate-400 uppercase tracking-wider">Belum ada riwayat audit</p>
            </div>
          ) : (
            <div className="space-y-3">
              {[...dismissedAlerts].reverse().map((da) => (
                <div 
                  key={da.id}
                  className="p-4 rounded-2xl border bg-slate-850/40 border-slate-800 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="text-[10px] px-2 py-0.5 rounded font-black uppercase tracking-wider bg-indigo-500/10 text-indigo-450 border border-indigo-500/20">
                        {da.title}
                      </span>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-450 hover:text-indigo-400 font-bold text-[10.5px] rounded-lg transition shrink-0 cursor-pointer"
                  >
        ...
        <div className="pt-4 border-t border-slate-800 flex justify-end shrink-0">
          <button
            type="button"
            onClick={() => setShowHistoryModal(false)}
            className="px-4 py-2 bg-slate-800 hover:bg-slate-700 font-bold rounded-xl text-xs text-slate-200 cursor-pointer"
          >
            Tutup
          </button>
        </div>
  ...
  const renderDismissConfirmModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xs animate-fade-in no-print">
        <div className="bg-slate-1000 border border-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl relative animate-scale-in">
          <div className="flex items-start gap-3">
            <div className="bg-amber-500/10 border border-amber-500/20 p-2 text-amber-500 rounded-xl shrink-0">
              <AlertTriangle className="h-6 w-6" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-white">Selesaikan Peringatan</h3>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">Apakah Anda yakin ingin menutup notifikasi ini? Ini hanya akan ditandai sebagai "sudah ditinjau" tanpa menghapus data pesanan.</p>
            </div>
          </div>
  ...
  const renderDismissAllConfirmModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xs animate-fade-in no-print">
        <div className="bg-slate-1000 border border-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl relative animate-scale-in">
          <div className="flex items-start gap-3">
            <div className="bg-rose-500/10 border border-rose-500/20 p-2 text-rose-500 rounded-xl shrink-0">
              <AlertTriangle className="h-6 w-6 animate-pulse" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-white">Tandai Semua Sudah Ditinjau</h3>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">Yakin menutup semua notifikasi Auditor yang masih aktif sekarang?</p>
            </div>
          </div>
          <div className="flex items-center justify-end gap-2.5 mt-5">
            <button
    ...
    <div className="bg-slate-950 rounded-2xl p-6 border border-slate-800 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <div className="bg-rose-500/10 border border-rose-500/20 p-2.5 rounded-xl text-rose-500 shrink-0">
            <ShieldAlert className="h-6 w-6" />
          </div>
          <div>
            <h2 className="text-xl font-extrabold text-white flex flex-wrap items-center gap-2">
              AI Auditor Internal & Deteksi Fraud
              <span className="text-[10px] px-2 py-0.5 bg-rose-600/35 text-rose-450 border border-rose-500/30 rounded-full font-black animate-pulse">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-850 text-slate-300 border border-slate-800 text-[10px] font-black uppercase tracking-wider rounded-xl cursor-pointer transition shadow-3xs"
          >
